...
class DashboardController extends AppController {
    ...
    public function calendar($month = null) {
        ...
        $view['schedules'] = \App\Schedule::whereRaw("EXTRACT(YEAR_MONTH FROM day) = '$month_raw'")->orderBy("hour_start")->orderBy("id")->get();
        foreach ($view['schedules'] as $j=>$schedule) {
            $pos = date('d', strtotime($schedule->day))+0;
            array_push($view['days'][$pos], $schedule);
        }

        $view['places'] = \App\Place::where("group_id", $group_id)->orderBy("name")->pluck('name', 'id');

        $view['prev'] = url("dashboard/".date('Y-m', strtotime("-1 month", strtotime($month))) );
        $view['next'] = url("dashboard/".date('Y-m', strtotime("+1 month", strtotime($month))) );

        return view('dashboard/mes', $view);
    }
